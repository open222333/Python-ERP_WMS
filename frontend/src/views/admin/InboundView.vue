...
const createForm = ref({ warehouse_id: '', supplier: '', remark: '' })

// Detail
const showDetail  = ref(false)
const detailOrder = ref<InboundOrder | null>(null)
const detailItems = ref<InboundItem[]>([])
...
async function openDetail(order: InboundOrder) {
  detailOrder.value = order
  detailItems.value = []
  showDetail.value  = true
  try {
    const { data } = await http.get(`/api/inbound/${order._id}`)
...
async function doAction(action: string) {
  ...
  try {
    await http.post(`/api/inbound/${detailOrder.value._id}/${action}`)
...
async function createOrder() {
  ...
  saving.value = true
  try {
    await http.post('/inbound/', createForm.value)
...
async function addItem() {
  ...
  try {
    await http.post(`/api/inbound/${detailOrder.value._id}/item`, itemForm.value)
...
function openCreateModal() {
  createForm.value = { warehouse_id: '', supplier: '', remark: '' }
          ...
          <div class="modal-body">
            <div class="mb-3">
              <label class="form-label fw-semibold">
                入庫倉庫 <span class="text-danger">*</span>
              </label>
            ...
            <div class="mb-3">
              ...
              <input v-model="createForm.supplier" type="text" class="form-control" placeholder="選填" />
            </div>
            <div class="mb-3">
        ...
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title">
              入庫單 #{{ detailOrder.order_no || detailOrder._id.slice(-6) }}
          ...
          <div class="modal-body">
            ...
            <div class="row g-2 mb-3 small text-muted">
              ...
              <div class="col-md-3">
                <i class="bi bi-truck me-1"></i>{{ detailOrder.supplier || '未填' }}
              </div>
              <div class="col-md-3">
              ...
              <table class="table table-sm mb-0">
                ...
                <tbody>
                  ...
                  <tr v-for="item in detailItems" :key="item._id">
                    ...
                    <td class="text-end">
                      ${{ Number((item.received_qty || item.expected_qty) * (item.unit_price ?? 0)).toFixed(2) }}
                    </td>
                    <td>
                      <button
                        v-if="['pending', 'confirmed'].includes(detailOrder.status)"
                  ...
                  <tr>
                    ...
                    <td class="text-end fw-bold text-primary">
                      ${{ detailItems.reduce((s, i) =>
                        s + (i.received_qty || i.expected_qty) * (i.unit_price ?? 0), 0
                      ).toFixed(2) }}
                    </td>
                    <td></td>
                  </tr>
