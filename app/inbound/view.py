    ...
    responses:
      201:
        ...
        schema:
          properties:
            success: {type: boolean}
            id: {type: string, description: 新入庫單 ID}
      400:
        description: 缺少必填欄位
      404:
        description: 倉庫不存在
...
def update_order(oid):
    ...
    parameters:
      ...
      - in: body
        ...
        schema:
          properties:
            supplier: {type: string, description: 供應商名稱}
...
def add_item(oid):
    ...
    responses:
      200:
        description: 新增成功
      400:
        description: 缺少產品 ID 或入庫單非 pending 狀態
      404:
        description: 產品不存在
    """
    data = request.get_json(silent=True) or {}
...
def update_item(oid, item_id):
    ...
    parameters:
      ...
      - in: body
        ...
        schema:
          properties:
            expected_qty: {type: integer, description: 預計入庫數量}
...
def complete_order(oid):
    """
    完成入庫（confirmed → completed），自動入帳庫存
    ---
    tags:
      - 入庫管理
    security:
    ...
    parameters:
      ...
      - in: body
        name: body
        description: 選填。不帶則以各明細 expected_qty 作為實收數量
        schema:
          properties:
            received_qtys:
              type: object
              description: "覆寫各明細實收數量，格式：{item_id: qty}"
              example: {"<item_id>": 9}
    responses:
      200:
        description: 完成成功
...
def cancel_order(oid):
    """
    取消入庫單（pending / confirmed 均可取消，completed 不可）
    ---
    tags:
      - 入庫管理
    security:
