...
- **QR 碼管理**：桌號識別碼（SKU 式 `T-001`，自動生成可手動修改）+ 輪替安全 Token 雙層設計；後台設定各桌 Token TTL（小時）、手動刷新、啟用 / 停用個別桌號；Token 懶觸發自動刷新；列表即時顯示各桌目前 Session 活躍狀態（有無顧客在線）與到期時間，支援手動強制結束指定桌況 Session（即時觸發顧客端「感謝光臨」提示）；標題列顯示「N 桌活躍中」統計；Session 狀態每 30 秒自動刷新，並提供「Session 狀態」手動刷新按鈕
- **後台安全**：登入 API 速率限制（10 次/分鐘；50 次/小時），Redis 跨 Worker 共享計數；`__guest__` 系統帳號自動建立，`locked` 鎖定不可刪改
- **廚房看板**：即時顯示待處理 / 處理中訂單（先進先出），無需後台登入；每張訂單卡片顯示已等待時間（正確處理 UTC 時區，每秒即時更新）
- **系統設定**：POS 預設菜單、操作紀錄保留天數、手動清除、後台外觀配色（☀ 淺色 / 🌙 深色模式 + 8 組側欄主題預設 + 自訂 Hex 色碼）
- **Swagger UI**：自動產生 API 文件
- **Docker 部署**：一鍵啟動（nginx + Flask + MongoDB + Redis）；純主機部署（gunicorn + nginx）
...
REDIS_HOST=redis
REDIS_PORT=6379
REDIS_PASSWORD=      # 留空（Redis 僅限內網，無需密碼）
REDIS_DB=0
```
...
| `redis` | redis:7-alpine | 僅內部 | 速率限制計數器（登入保護） |
...
編輯 `conf/nginx/conf.d/default.conf.cloudflare.template`，搜尋取代：
- `admin.example.com` → 實際後台域名
- `order.example.com` → 實際點餐域名（不需時可刪除對應 server block）
...
每個設定檔均包含兩個 `server {}` 區塊（預設使用 `admin.example.com` / `order.example.com` 作為範例域名，部署前搜尋取代為實際值）：
- **後台管理**（`admin.example.com`）— 完整入口（所有 API + Vue SPA）
- **顧客點餐**（`order.example.com`）— 僅 `/customer-order/`，其餘路由回 403；不需獨立點餐域名時可刪除此 block
...
設定檔使用字面值範例（`admin.example.com` / `order.example.com` / `/opt/wms`），複製前在編輯器中搜尋取代為實際值：

```bash
# ── 模式一：HTTP ────────────────────────────────────────────
# 編輯 conf/nginx/host/http.conf，搜尋取代：
#   admin.example.com → 後台管理域名（或 _ 接受所有請求）
#   order.example.com → 顧客點餐域名
#   /opt/wms          → 專案根目錄（含 frontend-dist/）
sudo cp conf/nginx/host/http.conf /etc/nginx/sites-available/flask-app

# ── 模式二：Cloudflare Origin CA SSL ──────────────────────
# 前置：建立 Cloudflare 憑證（參見檔案頂部說明；萬用字元憑證 *.example.com 可覆蓋兩個域名）
# 編輯 conf/nginx/host/cloudflare.conf，同上搜尋取代後：
sudo cp conf/nginx/host/cloudflare.conf /etc/nginx/sites-available/flask-app

# ── 模式三：Let's Encrypt SSL ──────────────────────────────
sudo apt install -y certbot python3-certbot-nginx
# 先以模式一（HTTP）啟動後申請憑證：
sudo certbot certonly --nginx -d admin.example.com   # 替換為實際域名
sudo certbot certonly --nginx -d order.example.com   # 替換為實際域名
# 編輯 conf/nginx/host/https-letsencrypt.conf，同上搜尋取代後：
...
| Collection | 說明 |
|---|---|
| `users` | 使用者帳號（bcrypt 加密密碼）+ `page_permissions` 頁面模板；`locked: true` 為系統保護帳號（`__guest__`）不可刪改 |
| `logs` | 所有操作紀錄（支援自動清除） |
